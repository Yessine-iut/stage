...
class PolitenessFeatureVectorizer:
    ...
    def __init__(self):
        ...
        """
        self.unigrams = cPickle.load(open(self.UNIGRAMS_FILENAME, 'rb'))
        self.bigrams = cPickle.load(open(self.BIGRAMS_FILENAME, 'rb'))
...
if __name__ == "__main__":
    ...
    # print(vectorizer)
    doc_text = ['If you have uploaded other non-free media, consider checking that you have specified how these images fully satisfy our non-free content criteria. You can find a list of description pages you have edited by clicking on']
    # f = vectorizer.features(doc)
    # print(f)
    # exit()

    for doc in [doc_text]:#TEST_DOCUMENTS:
        f = vectorizer.features(doc)
        # print(f)
        # exit()

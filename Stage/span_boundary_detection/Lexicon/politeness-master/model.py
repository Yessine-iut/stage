...
# clf = cPickle.load(open(MODEL_FILENAME,'rb'))
clf = cPickle.load(open(MODEL_FILENAME,'rb'),encoding='latin1')
# ._Unpickler(f)
...
def score(request):
    ...
    fv = [features[f] for f in sorted(features.iterkeys())] #features
    print(fv)
    exit()
...
if __name__ == "__main__":
    ...
    doc = ['If you have uploaded other non-free media, consider checking that you have specified how these images fully satisfy our non-free content criteria. You can find a list of description pages you have edited by clicking on']
